      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex items-center justify-around py-2 pb-4 z-50 shadow-lg">
        {[
          ...
          >
            ...
            {key === "active_issues" && activeFindings.length > 0 && (
              <span className="absolute top-1 bg-red-600 text-white text-[7px] w-3.5 h-3.5 rounded-full flex items-center justify-center font-black">
          ...
          <div className="space-y-4">
            ...
            {inspections.length === 0 ? (
              ...
              inspections.map(insp => {
                ...
                const closedCount = inspFindings.filter(f => f.is_closed).length;
                const openCount = inspFindings.filter(f => !f.is_closed).length;
                      ...
                      <div className="px-4 md:px-6 pb-6 border-t border-slate-50 pt-4 space-y-4">
                        {insp.summary && (
                          <div className="bg-slate-900 rounded-2xl p-5 text-white">
                            <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-2">Resumen Ejecutivo</p>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {inspFindings.map(f => (
                              <div
                                ...
                                className={`p-4 rounded-xl border-2 cursor-pointer hover:scale-[1.01] transition-all ${
                                  f.is_closed ? "bg-green-50/50 border-green-100" : "bg-red-50/50 border-red-100"
                                }`}
                              >
...
function ZoneMap({ zones, onZoneClick, backgroundImage, isDisabled }: {
  ...
  const getStyle = (status: ZoneStatus, hasIssues: boolean) => {
    if (hasIssues || status === "ISSUE") return "bg-red-500/40 border-red-600 text-red-900 border-2 animate-pulse";
    if (status === "OK") return "bg-green-500/30 border-green-600 text-green-900";
    return "bg-slate-200/60 border-slate-400 text-slate-700 hover:bg-blue-200/50 hover:border-blue-400";
  };

  return (
    <div className={`relative w-full aspect-video bg-slate-100 rounded-2xl border-4 border-slate-200 overflow-hidden shadow-inner ${isDisabled ? "opacity-70" : ""}`}>
      {backgroundImage ? (
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center opacity-10">
          <svg className="w-16 h-16 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>
          <p className="text-xs font-black uppercase tracking-widest">Sin plano de planta</p>
        </div>
      )}

      {zones.map(zone => {
        const hasIssues = Object.values(zone.findings).some((f: any) => !f.is_closed);
        return (
          <button
            key={zone.id}
            disabled={isDisabled}
            onClick={() => onZoneClick(zone)}
            className={`absolute border rounded-xl flex flex-col items-center justify-center text-center transition-all p-1 ${getStyle(zone.status, hasIssues)} ${isDisabled ? "cursor-not-allowed" : "cursor-pointer"}`}
            style={{ left: `${zone.x}%`, top: `${zone.y}%`, width: `${zone.width}%`, height: `${zone.height}%` }}
          >
            <span className="text-[7px] md:text-[10px] font-black uppercase leading-tight">{zone.name}</span>
            {zone.status === "OK" && <span className="text-[6px] md:text-[9px]">✓</span>}
            {(zone.status === "ISSUE" || hasIssues) && <span className="text-[6px] md:text-[9px]">⚠</span>}
          </button>
        );
      })}
      ...
      <div className="hidden md:flex absolute bottom-3 right-3 bg-black/60 backdrop-blur px-3 py-1.5 rounded-xl text-[8px] text-white font-black gap-4 border border-white/20">
        ...
        <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-red-500 inline-block"></span>HALLAZGO</span>
      </div>
    </div>
  );
}
...
// ─── Inspection Modal (Zone Checklist) ────────────────────────────────────────
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-50 p-2 md:p-6">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[95vh] flex flex-col border-4 border-slate-100">
        <div className="p-4 md:p-6 bg-slate-50 border-b flex justify-between items-center shrink-0">
          <div>
            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${hasAnyFail ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"}`}>
              {hasAnyFail ? "Con Hallazgos" : "Preventivo"}
            </span>
                    ...
                    {group.items.map(item => (
                      ...
                      >
                        <div className={`mt-0.5 w-6 h-6 rounded-lg flex items-center justify-center shrink-0 border-2 transition-all ${results[item.id] === false ? "bg-red-600 border-red-600 text-white" : "border-slate-300"}`}>
                          {results[item.id] === false ? <span className="text-[10px]">✕</span> : <span className="text-[10px] text-slate-300">✓</span>}
                        </div>
                        <div>
                          ...
                          {results[item.id] === false && findings[item.id] && (
                            <p className="text-[9px] text-red-500 font-bold mt-1">📷 Evidencia registrada</p>
                          )}
                        </div>
                      </div>
                    ))}
        ...
        <div className="p-4 md:p-6 bg-slate-50 border-t shrink-0">
          ...
          >
            {isSaving ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> GUARDANDO...</> : "CONFIRMAR INSPECCIÓN"}
          </button>
        </div>
      </div>
...
// ─── Finding Detail Modal (report new finding) ────────────────────────────────
function FindingDetailModal({ item, inspectionId, existing, onSave, onClear, onCancel }: {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border-4 border-red-50 overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-5 bg-red-50 border-b flex justify-between items-center shrink-0">
          <div>
            ...
            <h3 className="text-base font-black text-slate-800 mt-1 leading-tight">{item.label}</h3>
          </div>
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur flex items-center justify-center z-[150] p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col border-4 border-slate-100">
        <div className={`p-5 border-b flex justify-between items-start ${finding.is_closed ? "bg-green-50" : "bg-red-50"}`}>
          <div>
            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full text-white ${finding.is_closed ? "bg-green-600" : "bg-red-600"}`}>
              {finding.is_closed ? "RESUELTO" : "PENDIENTE"}
            </span>
            <h3 className="text-lg font-black text-slate-800 mt-2 leading-tight">{finding.item_label}</h3>
          </div>
          <button onClick={onClose} className="w-8 h-8 bg-white border rounded-lg flex items-center justify-center text-slate-400">✕</button>
        </div>
        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-4">
          ...
          {finding.photo_url && (
            <div className="cursor-zoom-in rounded-xl overflow-hidden border-4 border-slate-50 shadow" onClick={() => onImageZoom?.(finding.photo_url!)}>
              <img src={finding.photo_url} alt="Evidencia" className="w-full object-cover max-h-48" />
            </div>
          )}
          ...
          {finding.is_closed && finding.corrective_actions && (
